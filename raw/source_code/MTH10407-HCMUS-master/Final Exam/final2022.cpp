...
public:
    ...
    int pop() {
        ...
        throw runtime_error("Stack is empty"); // [FIX] tránh undefined behavior: trả về gì khi rỗng?
    }

    int top() {
        if (!isEmpty())
            return data.back();
        throw runtime_error("Stack is empty"); // [FIX] tránh undefined behavior
    }
...
public:
    ...
    virtual ~Shape() {} // [FIX] virtual destructor: tránh memory leak khi dùng Shape* shapes[]
};
