...
// [FIX] Thêm virtual destructor để tránh memory leak khi delete qua base pointer
// [FIX] Đổi thành pure virtual (= 0) vì Shape là abstract class, không nên tạo trực tiếp
class Shape {
public:
	virtual string printAllInformation() = 0; // [FIX] = 0: pure virtual
	virtual ~Shape() {}                        // [FIX] virtual destructor
};
...
public:
	...
	float getPerimeter() {
		float ans = 2 * M_PI * _radius; // [FIX] dùng M_PI thay 3.14 (chính xác hơn)
		return ans;
	}
	float getArea() {
		float ans = (float)(M_PI * _radius * _radius); // [FIX] M_PI
		return ans;
	}
...
public:
	void readFile(string fileName, vector<Shape*>& shapes) {
		...
		if (file.is_open()) {
			while (getline(file, line)) { // [FIX] tránh antipattern while(!eof): đọc dư 1 dòng rỗng cuối file
